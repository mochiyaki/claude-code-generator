...
async function main() {
  ...
  switch (command) {
    ...
    case "code":
      if (!isRunning) {
        ...
        if (await waitForService()) {
          ...
          console.error(
            "Service startup timeout, please manually run `ccg start` to start the service"
          );
          process.exit(1);
        }
      } else {
      ...
      if (!isRunning) {
        ...
        if (!(await waitForService())) {
          ...
          try {
            ...
            console.log(
              "Created minimal default configuration file at ~/.claude-code-generator/config.json"
            );
            console.log(
            ...
            if (!(await waitForService(15000))) {
              ...
              console.error(
                "Service startup still failing. Please manually run `ccg start` to start the service and check the logs."
              );
              process.exit(1);
            }
          } catch (error: any) {
    ...
    case "-v":
    case "version":
      console.log(`claude-code-generator version: ${version}`);
      break;
    case "restart":
