...
export async function checkForUpdates(currentVersion: string) {
  try {
    ...
    const { stdout } = await execPromise("npm view @gguf/claude-code-generator version");
...
export async function performUpdate() {
  try {
    // 执行npm update命令
    const { stdout, stderr } = await execPromise("npm update -g @gguf/claude-code-generator");
